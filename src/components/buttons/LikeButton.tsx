...
const LikeButton: React.FC<LikeButtonProps> = ({ liked, likes, handleLike }) => {
  return (
    <button
      onClick={handleLike}
      className={`mt-4 flex items-center ${liked ? "text-red-500" : "text-gray-400"} text-sm`}
    >
      {likes}
      <span className={`ml-2 ${liked ? "text-red-500" : "text-gray-400"}`}>
        {liked ? (
          <svg
